<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('kehoupingjia','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('kehoupingjia','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('kehoupingjia','课程满意度')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						课程满意度
					</el-button>
					<el-button class="btn18" v-if="isAuth('kehoupingjia','教练满意度')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						教练满意度
					</el-button>
					<el-button class="btn18" v-if="isAuth('kehoupingjia','体验感满意度')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						体验感满意度
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="课程满意度">
						<template slot-scope="scope">
							{{scope.row.kechengmanyidu}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiaolianpingfen"
						label="教练评分">
						<template slot-scope="scope">
							{{scope.row.jiaolianpingfen}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="tiyangan"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kehoupingjia','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('kehoupingjia','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('kehoupingjia','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="kechengmanyiduChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="jiaolianpingfenChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="tiyanganChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var kechengmanyiduChart1 = echarts.init(document.getElementById("kechengmanyiduChart1"),'macarons');
        this.$http({
            url: "kehoupingjia/group/kechengmanyidu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].kechengmanyidu);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].kechengmanyidu
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                kechengmanyiduChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kechengmanyiduChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var jiaolianpingfenChart2 = echarts.init(document.getElementById("jiaolianpingfenChart2"),'macarons');
        this.$http({
            url: "kehoupingjia/group/jiaolianpingfen",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].jiaolianpingfen);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].jiaolianpingfen
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                jiaolianpingfenChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    jiaolianpingfenChart2.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var tiyanganChart3 = echarts.init(document.getElementById("tiyanganChart3"),'macarons');
        this.$http({
            url: "kehoupingjia/group/tiyangan",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                tiyanganChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    tiyanganChart3.resize();
                };
            }
        });
      })
	...
	kehoupingjiastatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kehoupingjia/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
