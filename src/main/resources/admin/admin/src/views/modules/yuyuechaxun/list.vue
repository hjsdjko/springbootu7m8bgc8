<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 0 20px","borderRadius":"0px","textAlign":"left","background":"none","display":"block","width":"100%","fontSize":"inherit","order":"2"}' >
					...
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">手机号</label>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('yuyuechaxun','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yuyuechaxun','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('yuyuechaxun','课程预约率')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						课程预约率
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuyuechaxun','课程类型率')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						课程类型率
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuyuechaxun','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyuechaxun','退课')" @click="tuikechaxunCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								退课
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyuechaxun','评价')" @click="kehoupingjiaCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								评价
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyuechaxun','取消课程')" @click="quxiaochaxunCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								取消课程
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuyuechaxun','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yuyuechaxun','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<tuikechaxun-cross-add-or-update v-if="tuikechaxunCrossAddOrUpdateFlag" :parent="this" ref="tuikechaxunCrossaddOrUpdate"></tuikechaxun-cross-add-or-update>
		<kehoupingjia-cross-add-or-update v-if="kehoupingjiaCrossAddOrUpdateFlag" :parent="this" ref="kehoupingjiaCrossaddOrUpdate"></kehoupingjia-cross-add-or-update>
		<quxiaochaxun-cross-add-or-update v-if="quxiaochaxunCrossAddOrUpdateFlag" :parent="this" ref="quxiaochaxunCrossaddOrUpdate"></quxiaochaxun-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="kechengleixingChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import tuikechaxunCrossAddOrUpdate from "../tuikechaxun/add-or-update";
import kehoupingjiaCrossAddOrUpdate from "../kehoupingjia/add-or-update";
import quxiaochaxunCrossAddOrUpdate from "../quxiaochaxun/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				tuikechaxunCrossAddOrUpdateFlag: false,
				kehoupingjiaCrossAddOrUpdateFlag: false,
				quxiaochaxunCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			tuikechaxunCrossAddOrUpdate,
			kehoupingjiaCrossAddOrUpdate,
			quxiaochaxunCrossAddOrUpdate,
		},
		methods: {
			...
			tuikechaxunCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.tuikechaxunCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyuechaxun');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tuikechaxunCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tuikechaxunCrossaddOrUpdate.init(row.id,type);
				});
			},
			kehoupingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.kehoupingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyuechaxun');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kehoupingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kehoupingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
			quxiaochaxunCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.quxiaochaxunCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyuechaxun');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.quxiaochaxunCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.quxiaochaxunCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "yuyuechaxun/group/kechengmingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var kechengleixingChart2 = echarts.init(document.getElementById("kechengleixingChart2"),'macarons');
        this.$http({
            url: "yuyuechaxun/group/kechengleixing",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                kechengleixingChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kechengleixingChart2.resize();
                };
            }
        });
      })
	...
	yuyuechaxunstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'yuyuechaxun/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
