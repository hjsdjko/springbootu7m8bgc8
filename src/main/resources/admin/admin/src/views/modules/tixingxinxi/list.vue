		...
		<template v-if="showFlag">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="tixingfengmian" width="200" label="提醒封面">
						<template slot-scope="scope">
							<div v-if="scope.row.tixingfengmian">
								<img v-if="scope.row.tixingfengmian.substring(0,4)=='http'" :src="scope.row.tixingfengmian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.tixingfengmian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
...
import AddOrUpdate from "./add-or-update";
	...
	tixingxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'tixingxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
