	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="分享标题" prop="fenxiangbiaoti">
					<el-input  v-model="ruleForm.fenxiangbiaoti" placeholder="分享标题" clearable  :readonly="ro.fenxiangbiaoti"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="分享标题" prop="fenxiangbiaoti">
					<el-input v-model="ruleForm.fenxiangbiaoti" placeholder="分享标题" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-if="type!='info' && !ro.fenxiangfengmian" label="分享封面" prop="fenxiangfengmian">
					<file-upload
						tip="点击上传分享封面"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-else-if="ruleForm.fenxiangfengmian" label="分享封面" prop="fenxiangfengmian">
					...
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.fenxiangfengmian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="分享时间" prop="fenxiangshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.fenxiangshijian" label="分享时间" prop="fenxiangshijian">
					<el-input v-model="ruleForm.fenxiangshijian" placeholder="分享时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="分享详情" prop="fenxiangxiangqing">
					<editor 
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.fenxiangxiangqing" label="分享详情" prop="fenxiangxiangqing">
                    <span :style='{"padding":"0 20px","color":"inherit","background":"#fff","display":"inline-block","width":"80%","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.fenxiangxiangqing"></span>
                </el-form-item>
...
export default {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='fenxiangfengmian'){
							this.ruleForm.fenxiangfengmian = obj[o];
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.fenxiangxiangqing = this.ruleForm.fenxiangxiangqing.replace(reg,'../../../springbootu7m8bgc8/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.fenxiangfengmian!=null) {
		this.ruleForm.fenxiangfengmian = this.ruleForm.fenxiangfengmian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaoyouquan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaoyouquanCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    fenxiangfengmianUploadChange(fileUrls) {
	    this.ruleForm.fenxiangfengmian = fileUrls;
    },
  }
};
</script>
